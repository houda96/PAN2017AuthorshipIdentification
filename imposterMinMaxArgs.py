...
def imposterMethod(newDataDict, option, k, n, modeFeats, amountRuns, imposters, amountImposters):
	...
	for currentProbl in allProbl:
		if currentProbl not in argument_problems:
			print("skipping {}".format(currentProbl))
			continue
		print("{} Starting {}".format(datetime.datetime.now(), currentProbl))
...
def imposterFunc(doc1Feats, doc2Feats, imposters, amountRuns, amountFeatures, cv_fit,
	...
	for setVal in featureSets:
		...
		sim = minMaxDistance(useDoc1, useDoc2)
		for impost in impostersDoc1:
			sim1 = minMaxDistance(useDoc1, impost[setVal:setVal+amountFeatures])
			if sim > sim1:
				score += 1
		for impost in impostersDoc2:
			sim1 = minMaxDistance(useDoc2, impost[setVal:setVal+amountFeatures])
			if sim > sim1:
				score += 1
	return score/(amountRuns*len(impostersDoc1)*2)
...
def getAllImposters():
	infoFile = glob.glob("impostersShort/info.json")[0]
	with open(infoFile) as data_file:
		data = json.load(data_file)
...
def applyImposter(firstPath, outputPath, threshold, amountRuns, option, k, n, mode, amountImposters):
	...
	#with open('outcomeImposters.txt', 'wb') as handle:
  		#pickle.dump(allImposters, handle)
	print "Reading imposters.."
	with open('outcomeImposters.txt', 'rb') as handle:
  		allImposters = pickle.loads(handle.read())
	allInfo = imposterMethod(newDataDict, option, k, n, mode, amountRuns, allImposters, amountImposters)
...
if __name__ == "__main__":
	if len(sys.argv) != 6:
		print("imposter.py -i EVAL_DIRECTORY -o OUTPUT_DIRECTORY RANGES")
	firstPath = sys.argv[2]
	outputPath = sys.argv[4]
	option = "normal"

	argument_problems = ["problem{:03d}".format(i) for r in (x.split("-") for x in sys.argv[5].split(",")) for i in range(int(r[0]), int(r[-1]) + 1)]
    # roep aan met allExperiments/compareImposter
	# 1 -> hogere threshold

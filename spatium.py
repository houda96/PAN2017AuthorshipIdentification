    ...
    for prob in problems:
        ...
        for doc1 in allDocs:
            ...
            for doc2 in otherDocs:
                ...
                doc1Dict[doc2] = score
            minScore[doc1] = min(allScore)
            maxScore[doc1] = max(allScore)
            symmetricScores[doc1] = doc1Dict
        allSymmetricScores[prob] = symmetricScores
        allMinScores[prob] = minScore
        allMaxScores[prob] = maxScore
    return allSymmetricScores, allMaxScores, allMinScores

def getAllLinks(problems, allInfo, allSymmetricScores, allMinScores,
                allMaxScores, threshold):
    """ Given a list with the names of a problem set, a dictionary with the
    ...
    for prob in problems:
        ...
        currentKeys = []
        for doc1 in allDocs:
            minScore = allMinScores[prob][doc1]
            maxScore = allMaxScores[prob][doc1]
            for doc2 in allDocs:
                if doc1 != doc2:
                    ...
                    if (doc2, doc1) not in currentKeys:
                        ...
                        if score > threshold:
                            if maxScore == minScore:
                                ...
                                newScore = (score - minScore)/(maxScore - minScore)
...
def applySpatium(firstPath, outputPath, threshold, option, k, n, modeFeats):
    ...
    allSymmetricScores, allMaxScores, allMinScores = getSymmetricScores(problems,
                                                        allInfo, allScores)
    eachLinks = getAllLinks(problems, allInfo, allSymmetricScores, allMinScores,
                            allMaxScores, threshold)
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("spatium.py -i EVAL_DIRECTORY -o OUTPUT_DIRECTORY")
    firstPath = sys.argv[2]
    outputPath = sys.argv[4]
    option = "top"
    applySpatium(firstPath, outputPath, 3, option, 250, 4, "char")
    # nVals = [4, 5, 6]
    # kVals = [1, 10, 20, 30, 40, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500]
    # eVals = [1, 2, 3, 4, 5, 6, 7, 8, 9]
    # thresholds = [2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6]
    # for n in nVals:
    #     for k in kVals:
    #         for e in eVals:
    #             newPath = outputPath + "n" + str(n) + "c/k" + str(k)
    #             newPath += "/experiment" + str(e) + "/"
    #             applySpatium(firstPath, newPath, thresholds[e-1], option, k, n, "char")
    #             newPath1 = outputPath + "n" + str(n) + "w/k" + str(k)
    #             newPath1 += "/experiment" + str(e) + "/"
    #             applySpatium(firstPath, newPath1, thresholds[e-1], option, k, n, "word")
    #             print "finished", n, k, e
